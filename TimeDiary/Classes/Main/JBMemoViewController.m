...
{
    if (!_topToolBarView) {
        _topToolBarView = [[UIView alloc] initWithFrame:CGRectMake(0, kNavigationBarMaxY, JBScreenWidth, 35)];
...
- (UIButton *)setupTimeBtn
{
    if (!_setupTimeBtn) {
        ...
        _setupTimeBtn.frame = CGRectMake(5, 5+kNavigationBarMaxY, 143, 30);
...
-(UIButton *)Labelbtn {
    ...
    if (!_Labelbtn) {
        ...
        //适配plus
        if (kIsiPhoneX) {
            _Labelbtn.origin = CGPointMake(320, JBScreenHeight - 170);
        }else if(JBScreenWidth == 320 ) {
...
-(UIButton *)shotbtn {
    ...
    if (!_shotbtn) {
        ...
        //适配plus
        if (kIsiPhoneX) {
            _shotbtn.origin = CGPointMake(38, JBScreenHeight - 170);
        }else if(JBScreenWidth == 320 ) {
